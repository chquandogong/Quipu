...
CPU/Load/NVMe/Wi-Fi 被折叠成一个 `Telemetry Brief`，作为辅助信号。
DTIHAVR 也不再用 7 个大型固定步骤块展示，而是用轻量 `Workflow Rail`
...
CPU package、Load Average、NVMe 等核心指标不会只显示孤立数字。详细区的
`Metric Ledger` 会把核心指标放在同一个行式信息框中，每个信息按钮会用韩文
解释并保留英文技术术语，同时说明时间窗口、解读方式和下一步检查。例如 Load
会明确说明它是 Linux 1/5/15 分钟 load average，而不是瞬时 CPU 使用率。
CPU package 行会用编号 chip 显示每个 core 的温度，避免重复文字；Load 行会
显示 1/5/15 分钟窗口，NVMe 和 Wi-Fi 行会在代表值旁边显示每个设备或接口的读数。
...
SMART/NVMe health 之后也可以沿用同一 rule-based 结构扩展。`Telemetry Matrix`
里的 coverage chip 不是风险分数。`9/10 observed` 表示 10 个调查类别中有
9 个已经有可用 telemetry，另 1 个仍然 missing。
...
- `cpu.load_1m`, `cpu.load_5m`, `cpu.load_15m`: Linux 1/5/15 分钟 load average
- `memory.used_percent`: 基于 `/proc/meminfo` 的内存使用率
- `disk.root_used_percent`: root filesystem 使用率
- `cpu.package_temp_c`, `cpu.core_<n>.temp_c`, `thermal.*.temp_c`: sysfs thermal zone 和 hwmon CPU/core 温度
- `nvme.temp_c`, `nvme.<device>.temp_c`: 通过 hwmon 暴露的代表值和逐设备 NVMe 温度
...
- `wifi.signal_dbm`, `wifi.<interface>.signal_dbm`: 来自 `/proc/net/wireless` 的代表值和逐接口 Wi-Fi 信号
